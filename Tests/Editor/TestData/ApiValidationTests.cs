...
namespace UnityEditor.PackageManager.ValidationSuite.Tests
{
    internal class ApiValidationTests
    {
        internal const string testPackageRoot =
            "Packages/com.unity.package-validation-suite/Tests/Editor/TestData/AVAssemb/";
        [Test]
        ...
        public void AddingProperty_FailsInPatch(ReleaseType releaseType, bool expectError)
        {
            ...
            var apiValidation = Validate("PropAdd", releaseType);
        ...
        {
            List<string> messagesExpected = new List<string> { "Error: New assembly \"Unity.PackageValidationSuite.EditorTests.InternalOnlyAsmdefAdd.NewAsmdef\" may only be added in a new minor or major version." };
            var apiValidation = Validate("InternalOnlyAsmdefAdd", releaseType);
            ExpectResult(apiValidation, expectError, messagesExpected);
        }
        ...
        public void AddIncludePlatform_FailsInPatch(ReleaseType releaseType, bool expectError)
        {
            ...
            var apiValidation = Validate("WithTwoIncludePlatforms", releaseType, "WithIncludePlatform");
            ExpectResult(apiValidation, expectError, messagesExpected);
        }
        ...
        public void AddFirstIncludePlatform_FailsInMinor(ReleaseType releaseType, bool expectError)
        {
            ...
            var apiValidation = Validate("WithIncludePlatform", releaseType);
            ExpectResult(apiValidation, expectError, messagesExpected);
        }
        ...
        public void AddExcludePlatform_FailsInMinor(ReleaseType releaseType, bool expectError)
        {
            ...
            var apiValidation = Validate("WithExcludePlatform", releaseType);
            ExpectResult(apiValidation, expectError, messagesExpected);
        }
        ...
        public void RemoveExcludePlatform_FailsInPatch(ReleaseType releaseType, bool expectError)
        {
            ...
            var apiValidation = Validate("TestPackage_Base", releaseType, "WithExcludePlatform");
            ExpectResult(apiValidation, expectError, messagesExpected);
        }
        ...
        public void RemoveIncludePlatform_FailsInMinor(ReleaseType releaseType, bool expectError)
        {
            ...
            var apiValidation = Validate("WithIncludePlatform", releaseType, "WithTwoIncludePlatforms");
            ExpectResult(apiValidation, expectError, messagesExpected);
        }
        ...
        public void RemoveAllIncludePlatforms_FailsInMinor(ReleaseType releaseType, bool expectError)
        {
            ...
            var apiValidation = Validate("TestPackage_Base", releaseType, "WithIncludePlatform");
            ExpectResult(apiValidation, expectError, messagesExpected);
        }
        ...
        public void BreakingChange_FailsInMinor(ReleaseType releaseType, bool expectError)
        {
            ...
            var apiValidation = Validate("BreakingChange", releaseType);
            ExpectResult(apiValidation, expectError, messagesExpected);
        }
        ...
        public void RemovingAsmdef_FailsInMinor(ReleaseType releaseType, bool expectError)
        {
            ...
            var apiValidation = Validate("TestPackage_Base", releaseType, "AsmdefWithTypeAdd");
            ExpectResult(apiValidation, expectError, messagesExpected);
        }
        ...
        public void RenamingAsmdef_FailsInMinor(ReleaseType releaseType, bool expectError)
        {
            ...
            if (releaseType == ReleaseType.Patch)
            {
                ...
                {
                    "Error: Additions require a new minor or major version.",
                    "Error: Assembly \"Unity.PackageValidationSuite.EditorTests.RenamedAsmdef\" no longer exists or is no longer included in build. This requires a new major version.",
            ...
            var apiValidation = Validate("RenamedAsmdef", releaseType);
            ExpectResult(apiValidation, expectError, messagesExpected);
        }
        ...
        {
            List<string> messagesExpected = new List<string> { "Error: Package Validation Suite does not support .asmdefs that are not built on the \"Editor\" platform. See \"Unity.PackageValidationSuite.EditorTests.ExcludesEditor\"" };

            var apiValidation = Validate("ExcludesEditor", "0.1.0", "ExcludesEditor", copyAssemblies: false);
        ...
        {
            List<string> messagesExpected = new List<string> { "Error: Package Validation Suite does not support .asmdefs that are not built on the \"Editor\" platform. See \"Unity.PackageValidationSuite.EditorTests.IncludesAndroid\"" };

            var apiValidation = Validate("IncludesAndroid", "0.1.0", "IncludesAndroid", copyAssemblies: false);
            ExpectResult(apiValidation, true, messagesExpected);
        }
        ...
        public void AddingUpdaterConfig_FailsInMinor(ReleaseType releaseType, bool expectError)
        {
            ...
            var apiValidation = Validate("WithObsoleteUpdater", releaseType);
            ExpectResult(apiValidation, expectError, messagesExpected);
        }
