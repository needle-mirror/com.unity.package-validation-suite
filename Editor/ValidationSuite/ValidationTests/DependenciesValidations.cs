            ...
            foreach (var dependency in Context.ProjectPackageInfo.dependencies)
            {
                if (Context.PackageCoDependencies.ContainsKey(dependency.Key))
                {
                    ...
                    foreach (var verifiedDependency in verifiedDependencies)
                    {
                        ...
                        {
                            AddWarning("Package dependency {0}@{1} will create a major conflict with verified package {2}:{3}", dependency.Key, dependency.Value, verifiedDependency.ParentName, verifiedDependency.ParentVersion);
                        }
                    }
                }
            }
